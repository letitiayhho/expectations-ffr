...
#SBATCH --time=02:00:00 # 40 min enough for most? 2:20 max? trying 1:30
#SBATCH --partition=bigmem2
#SBATCH --ntasks=1
#SBATCH --mem-per-cpu=128G # 9git 6 enough for most, 128 max?
...
def main(sub, run):
    ...
    raw, events = raw.resample(5000, events = events) # resample to 2500 Hz
    ...
    raw_for_ffr = raw_for_ffr.filter(l_freq = FFR_PASSBAND[0], h_freq = FFR_PASSBAND[1])
    # then epoch
    epochs = mne.Epochs(
        raw_for_ffr,
        events,
